...
class Location extends Object {
  constructor(_details) {
    ...
    this.latitude = _details.latitude;
    this.longitude = _details.longitude;
  }
 ...
 */
Location.prototype.distance = (_location) => {
  if (!(_location instanceof Location)) ModelError.throwTypeMismatchError('Location');
  const thisLatitude = Utils.degreesToRadians(this._latitude);
  const givenLatitude = Utils.degreesToRadians(_location.latitude);
  const dLatitude = Utils.degreesToRadians(this._latitude - _location.latitude);
  const dLongitude = Utils.degreesToRadians(this._longitude - _location.longitude);
